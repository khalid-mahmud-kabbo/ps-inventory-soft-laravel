...
//         'Purchase',
//         'Sale',
//         'Sale Return',      // Included common types that involve a party
//         'Purchase Return',  // Included common types that involve a party
//     ];
...
//         'Sale Return',
//         'Purchase Return',
//         // Note: Expense and Purchase Order should likely be excluded or handled separately
//         // if they don't involve the 'Party' model for filtering.
//     ];

//     // --- 2. BASE QUERY & PARTY FILTER LOGIC ---

//     // Define the party filter closure to handle both party_id and party_type
//     $partyFilter = function (EloquentBuilder $q) use ($partyId, $partyType, $certainPartyRelatedMorphKeys) {
//         // Only apply the complex morph filter if a specific party ID or type is selected
//         if ($partyId || $partyType) {
//             $q->whereHasMorph(
//                 'transaction',
//                 $certainPartyRelatedMorphKeys,
//                 function ($r) use ($partyId, $partyType) {
//                     if ($partyId) {
//                         // Priority 1: Filter by specific Party ID
//                         $r->where('party_id', $partyId);
//                     } elseif ($partyType) {
//                         // Priority 2: Filter by Party Type (Applies when no Party ID is selected)
//                         // This assumes the underlying 'transaction' models (Sale, Purchase, etc.)
//                         // have a 'party' relationship defined pointing to the main Party model.
//                         $r->whereHas('party', fn($p) => $p->where('party_type', $partyType));
//                     }
//                 }
//             );
...
//                 ->orWhere('transfer_to_payment_type_id', $cashId);
//         })
//         // 💡 APPLY the combined party filter
//         ->when($partyId || $partyType, $partyFilter)
//         ->get()
//         ->map(fn($row) => tap($row, fn(&$r) => $r->flow_type = 'cash'));
...
//     // (This section remains largely the same, but the party logic handles the totals correctly based on filtered data)
...
//     $transactions = $transactions->map(function ($row) use (&$balance, &$sumCashIn, &$sumGrandTotal, &$sumCashOut, &$finalPartyLedgerBalance, &$finalBalanceStatus, &$finalBalanceClass, $dangerTypes, $partyId) {

//         // ... (Cash In / Cash Out logic remains the same) ...
//         $isCashIn = true;
//         if (!empty($row->type_of_payment)) {
//             $type = strtolower(trim($row->type_of_payment));
//             if ($type === 'receive') $isCashIn = true;
...
//         // ... (Total Due / Balance calculation remains the same) ...
//         $rawTotalDue = 0;
//         if ($row->transaction?->party) {
//             $row->balanceData = $this->partyService->getPartyBalance([$row->transaction->party->id]);
//             $rawTotalDue = $row->balanceData['balance'] ?? 0;
...
//         $balance += $row->balance;
//         $sumCashIn += $row->cash_in;
//         $sumGrandTotal += $grandTotal;
//         $sumCashOut += $row->cash_out;
...
//     // --- 6. DATATABLES OUTPUT ---
//     // (Aggregation and formatting remains the same)
//     $totalGrandTotal = $this->formatWithPrecision($sumGrandTotal, comma: false);
//     $totalCashIn = $this->formatWithPrecision($sumCashIn, comma: false);
//     $totalCashOut = $this->formatWithPrecision($sumCashOut, comma: false);
